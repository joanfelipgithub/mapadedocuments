javascript:(()=>{
  ...
  if (!document.getElementById('clickeduPersistent')) {
    const script = document.createElement('script');
    script.id = 'clickeduPersistent';
    script.textContent = `
      (function() {
        let loaded = false;
        ...
        function buildUI() {
          const waitForResults = setInterval(() => {
            ...
            if (rows.length && !loaded) {
              ...
              // Clean up
              sessionStorage.removeItem('clickeduActive');
              const persistScript = document.getElementById('clickeduPersistent');
              if (persistScript) persistScript.remove();
            }
          }, 500);
        }
        
        // Check if we're returning after search
        if (sessionStorage.getItem('clickeduActive') === 'true') {
          buildUI();
        }
      })();
    `;
